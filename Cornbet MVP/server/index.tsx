...
app.get(`${PREFIX}/ncaab-futures`, async (c) => {
  try {
    ...
    const FUTURE_MARKETS = [
      ...
      { sportKey: "basketball_ncaab_elite_eight",           market: "elite_eight",     label: "Make the Elite Eight" },
      { sportKey: "basketball_ncaab_sweet_sixteen",         market: "sweet_sixteen",   label: "Make the Sweet 16" },
    ];
    ...
    const fetchEspnFutures = async (): Promise<{ name: string; odds: string; market: string }[]> => {
      ...
      try {
        ...
        // Column index → market key (col 0 = team, col 2=championship, col 4=final_four, col 5=elite_eight, col 6=sweet_sixteen)
        ...
        while ((rowMatch = rowRegex.exec(html)) !== null) {
          ...
          // Team name is inside an <a> tag in cell 0
    ...
    // Merge: Odds API results take priority for championship; ESPN fills in the rest
    ...
    // For markets other than championship, prefer ESPN data (Odds API markets currently empty)
    // For championship, use Odds API (more accurate, real-time) and supplement with ESPN if missing
...
app.get(`${PREFIX}/admin/bets`, async (c) => {
  try {
    ...
    console.log(`GET /admin/bets: ${allBets.length} total across ${userIds.length} users`);
...
app.delete(`${PREFIX}/admin/bets`, async (c) => {
  try {
    ...
    // Refund stake to the player if bet is still pending
    ...
    if (existing.status === "pending" && betUserId) {
      ...
      if (stake > 0) {
        ...
        console.log(`DELETE /admin/bets: refunded $${stake} to userId=${betUserId} by admin=${adminId}`);
      }
    }

    await kv.del(kvKey);
    console.log(`DELETE /admin/bets: deleted key=${kvKey} by admin=${adminId}`);
